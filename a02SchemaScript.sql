...
Select  "Book_Dates" as "EventType",
concat(MonthName(X.ActivityDate), ',' ,
YEAR(X.ActivityDate)) as Month, 
Count(*) as Count from  ActivityHappens X, 
Eventbook Y where X.EventID = Y.EventID group by MonthName(X.ActivityDate);
   
Select  "Conference_Dates" as "EventType",
concat(MonthName(X.ActivityDate), ',' ,
YEAR(X.ActivityDate)) as Month, 
Count(*) as Count from  ActivityHappens X, 
EventConference Y where X.EventID = Y.EventID group by MonthName(X.ActivityDate);

Select  "Journal_Dates" as "EventType",
concat(MonthName(X.ActivityDate), ',' ,
YEAR(X.ActivityDate)) as Month, 
Count(*) as Count from  ActivityHappens X, 
EventJournal Y where X.EventID = Y.EventID group by MonthName(X.ActivityDate);

Select  "Book_Dates" as "EventType",
concat(MonthName(X.ActivityDate),' ', DAY(X.ActivityDate), ',' ,YEAR(X.ActivityDate))
as Month from  ActivityHappens X, EventBook Y
where X.EventID = Y.EventID and X.ActivityDate > 2017-01-01
order by X.ActivityDate Asc limit 1;

Select  "Conference_Dates" as "EventType",
concat(MonthName(X.ActivityDate),' ', DAY(X.ActivityDate), ',' ,YEAR(X.ActivityDate))
as Month from  ActivityHappens X, EventConference Y
where X.EventID = Y.EventID and X.ActivityDate > 2017-01-01
order by X.ActivityDate Asc limit 1;

Select  "Journal_Dates" as "EventType",
concat(MonthName(X.ActivityDate),' ', DAY(X.ActivityDate), ',' ,YEAR(X.ActivityDate))
as Month from  Activityhappens X, EventJournal Y
where X.EventID = Y.EventID and X.ActivityDate > 2017-01-01
order by X.ActivityDate Asc limit 1;